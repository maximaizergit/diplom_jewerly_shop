...
class CParser extends \CBitrixComponent
{
    ...
    public function prepareResult(){
        ...
        $this->arResult['url']=$request->get('url');
        $this->catalogClass =$request->get('catalog');
        $this->arResult['catalogClass']=$this->catalogClass;
        $this->filterLinks =$request->get('linksfilter');
        $this->arResult['filteredLinks']=$this->filterLinks;
        $this->arResult['ISPARSEDLINKS']=$request->get('isparsedlinks');
        $this->tags = $this->processText($request->get('tags'));
        $this->arResult['tags']=$request->get('tags');
        $this->codes = $this->convertStringToArray($request->get('codes'));
        $this->arResult['codes']=$request->get('codes');
    ...
    if (!empty($this->url)){
        ...
        if ($action =='parse'){
            ...
            if($validateRes = $this->validateTags()){
                ...
                $this->processedTags = $this->processText($request->get('tags'));

                $this->arResult['arrTags']= $this->processedTags;
                if(!empty($this->links)){
                    foreach ($this->links as $link){

                        $this->arResult['item'][] = $this->parseDetailPage($link);
                    }
                    $this->arResult['converteditems']=$this->mergeCodesAndItems($this->arResult['item'], $this->codes);
        ...
        if ($action == 'import'){
            if($validateRes = $this->validateTags()){
                ...
                $convertedItems = $this->mergeCodesAndItems($this->arResult['item'], $this->codes);
                $this->importToIblock($request->get('IID'), $convertedItems, $request->get('section'));
