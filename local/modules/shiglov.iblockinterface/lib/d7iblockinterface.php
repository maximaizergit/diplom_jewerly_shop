...
class D7IBlockInterface
{
    //CREATE

    public static function d7test($iblockID)
    {
        $dataClass = \Bitrix\Iblock\Iblock::wakeUp($iblockID)->getEntityDataClass();
        if ($dataClass) {

            $element = $dataClass::getByPrimary(815, [
                'select' => ['ID', 'NAME', 'DETAIL_TEXT', 'DETAIL_PICTURE', "BRAND_" => "BRAND"],
            ])->fetch();


            return $element;
        }
        return 'Отсутсвует символьный код API';
    }

    public static function getElementsArray($iblockID, $filter = ['=ACTIVE' => 'Y'], $props = [], $select = ['ID', 'NAME'], $price = false)
    {
        $dataClass = \Bitrix\Iblock\Iblock::wakeUp($iblockID)->getEntityDataClass();
        if ($dataClass) {
            if (!empty($props)) {
                $select = array_merge($select, $props);
            }
            ...
            if ($price) {
                ...
                foreach ($elements as $element) {
                    $element['PRICE_DATA'] = self::getElementPrice($element['ID']);
                    $resArr[] = $element;
                }
                $elements = $resArr;
            }
    ...
    public static function getElementArray($iblockID, $elementID, $props = [], $select = ['ID', 'NAME'], $price = false)
    {
        $dataClass = \Bitrix\Iblock\Iblock::wakeUp($iblockID)->getEntityDataClass();
        if ($dataClass) {
            if (!empty($props)) {
    ...
    public static function createElement($iblockID, $data = ['NAME' => 'Untitled'],$price = null, $quantity = null)
    {
        $dataClass = \Bitrix\Iblock\Iblock::wakeUp($iblockID)->getEntityDataClass();
        if ($dataClass) {
            ...
            if ($result->isSuccess()) {
                ...
                if ($price) {
                    self::setPrice($newElementId, $price);
                }
                if ($quantity) {
                    self::setQuantity($newElementId, $quantity);
                }
                return "Новый элемент успешно добавлен. ID: " . $newElementId;
            } else {
                $errors = $result->getErrors();
                foreach ($errors as $error) {
                    return "Ошибка при добавлении элемента: " . $error->getMessage();
                }
            }
        }
        return 'Отсутсвует символьный код API';
    ...
    public static function getElementsObjects($iblockID, $filter = ['=ACTIVE' => 'Y'], $props = [], $select = ['ID', 'NAME'])
    {
        $dataClass = \Bitrix\Iblock\Iblock::wakeUp($iblockID)->getEntityDataClass();
        if ($dataClass) {
            if (!empty($props)) {
    ...
    public static function getElementObject($iblockID, $elementID, $props = [], $select = ['ID', 'NAME'])
    {
        $dataClass = \Bitrix\Iblock\Iblock::wakeUp($iblockID)->getEntityDataClass();
        if ($dataClass) {
            if (!empty($props)) {
            ...
            $elements = $dataClass::getByPrimary($elementID, [
                'select' => $select,
            ])->fetchObject();
